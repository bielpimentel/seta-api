...
public class QRCodeController {
  ...
  public QRCodeController(QRTokenService qrTokenService, AccessLogService accessLogService) {
  ...
  public ResponseEntity<QRCodeDTO> generateQRCode(@CurrentUser User user) {
    ...
    boolean hasRecentAccess = accessLogService.hasRecentAccess(user);

    if (hasRecentAccess) return ResponseEntity.badRequest().build();
